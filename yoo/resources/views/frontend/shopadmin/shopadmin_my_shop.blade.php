            ...
            <div class="row mt-3">
                <div class="content-header d-flex justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="d-flex align-items-tcenter home-logo">
                            <small>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function getShopList(shop_type_id) {
                var url =
                    "{{ route('shopadmin.shopFetchGrid', ['type' => 'my', 'shop_type_id' => ':shopFetchGrid']) }}";
                url = url.replace(':shopFetchGrid', shop_type_id);
                $.ajax({
                    url: url,
                    method: "GET",
                    contentType: false,
                    ...
                    success: function(response) {
                        let mainContainer = $('#content-' + response.shop_type.id);
                        let btnName = $('#acc-btn-' + response.shop_type.id);
                        if (response.shop_list.length == 0) {
                            $(document.createElement('div')).addClass('empty-shop-' + response.shop_type
                                    .id).text('Shop List Empty...')
                                .appendTo(mainContainer);
                        } else {
                            $.each(response.shop_list, function(key, shop) {
                                shopCard(mainContainer, shop);
                            });
                            editBtn();
                            $("input.toggler").off('click').on('click', function() {
                                ...
                                let t_url =
                                    "{{ route('shopadmin.shopInfoToggleFetch', ':id') }}";
                                ...
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        ...
                                        $('#toggle_shop_info_id').val(response.shop
                                            .id);
                                        $('#toggle_status').val(status);
                                        if (status == 3) {
                                            $('#toggle-shop-modal-label').text(
                                                'Publish Shop')
                                            $('#toggle-shop-modal').on(
                                                'hidden.bs.modal',
                                                function() {
                                                    thisToggle.prop(
                                                        "checked",
                                                        false);
                                                    thisToggle.prop('disabled',
                                                        false);

                                                });
                                            $('#toggle-message').text(response
                                                .shop.name +
                                                ' will be published. Proceed?'
                                            );
                                        } else if (status == 4) {
                                            $('#toggle-shop-modal-label')
                                                .text(
                                                    'Deactivate Shop');
                                            $('#toggle-shop-modal').on(
                                                'hidden.bs.modal',
            ...
            $("#view-type").on('click', function() {
                ...
                if (view != 'table') {
                    ...
                    window.history.pushState({}, '',
                        "{{ route('shopadmin.shop', ['type' => 'my', 'view' => 'table']) }}");
                    ...
                    window.history.pushState({}, '',
                        "{{ route('shopadmin.shop', ['type' => 'my', 'view' => 'grid']) }}");
                }
            });
                            ...
                            if (response.place_id_null) {
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        var url =
                                            "{{ route('shopadmin.shopInfo', ['type' => 'my', 'id' => ':id']) }}";
                                ...
                                shopCard($('#content-' + response.shop_info.shop_type_id), response.shop_info);
                                $('#shop-list-table').DataTable().destroy();
                                shopListTable();
                                editShopBtn();
                            }
