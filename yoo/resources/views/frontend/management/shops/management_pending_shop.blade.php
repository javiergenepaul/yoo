...
@section('title', 'Pending Shop List Page')
                                ...
                                <div class="accordion-body">
                                    <div class="row" id="content-{{ $type->id }}">
                                        @if ($type->shopInfos->isNotEmpty())
                                            @foreach ($type->shopInfos->whereIn('shop_status_id', [1,2,5])->sortByDesc('id') as $shop)
                                                <div class="col-lg-2 col-md-3 col-sm-5 col-xs-6" id="shop-id-{{ $shop->id }}">
                                                    <div class="card">
                                                        ...
                                                        <div class="çard-footer p-3">
                                                            <a class="btn btn-primary" href="{{ route('management.shopInfo', ['type' => 'pending', 'id' => $shop->id]) }}" style="font-size: 15px;">
                                                                View More
                                                            </a>
                                                        </div>
                                                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function shopCard(content, shop) {
                ...
                shop_route = "{{ route('management.shopInfo', ['type' => 'my', 'id' => ':id']) }}";
            ...
            function shopListTable() {
                $('#shop-list-table').DataTable({
                    ...
                    ajax: {
                        type: "GET",
                        url: "{{ route('management.shopFetchTable', 'pending') }}",
                        dataType: "json",
            ...
            function editShopBtn() {
                $('.edit-btn').off('click').on('click', function() {
                    var shop_fetch_url = "{{ route('management.editShopFetch', ':shop_info_id') }}";
            ...
            $("#view-type").click(function() {
                ...
                if (view != 'table') {
                    ...
                    window.history.pushState({}, '',
                        "{{ route('management.shop', ['type' => 'pending', 'view' => 'table']) }}");
                    ...
                    window.history.pushState({}, '',
                        "{{ route('management.shop', ['type' => 'pending', 'view' => 'grid']) }}");
                }
            });
            ...
            $('#add-shop-form').validate({
                ...
                submitHandler: function(form, event) {
                    ...
                    $.ajax({
                        ...
                        complete: function() {
                            $('#add-shop-submit-btn').attr('disabled', false)
                                .text('submit');
                        },
                        success: function(response) {
                            if (response.place_id_null) {
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        var url =
                                            "{{ route('management.shopInfo', ['type' => 'pending', 'id' => ':id']) }}";
                                ...
                                let mainContainer = $('#content-' + response.shop_info
                                    .shop_type_id);
                                shopCard(mainContainer, response.shop_info, 'add');
                                $('#shop-list-table').DataTable().destroy();
                                shopListTable();
                                editShopBtn();
                            }
